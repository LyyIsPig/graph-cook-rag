...
class HybridRetrievalModule:
    ...
    def extract_query_keywords(self, query: str) -> Tuple[List[str], List[str]]:
        ...
        """
        prompt = f"""
提取查询关键词并返回JSON。不要输出代码或解释，只输出JSON对象。

查询：{query}

直接输出JSON（不要用代码块包裹）：
{{"entity_keywords": ["食材或菜品名"], "topic_keywords": ["主题概念"]}}

规则：
- entity_keywords: 具体食材、菜品名称（如["番茄","鸡蛋"]）
- topic_keywords: 抽象概念、菜系、风格（如["家常菜","快手菜"]）
"""
        ...
        try:
            ...
            import re
            json_match = re.search(r'\{.*"entity_keywords".*\}', content, re.DOTALL)
            if json_match:
